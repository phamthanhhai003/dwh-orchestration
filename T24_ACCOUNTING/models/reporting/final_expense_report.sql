...
-- Division: accounting-expense
-- Grain: 1 row per (load_date, branch_name, order_id)
-- 72 template rows (includes headers, data rows, subtotals, totals) per branch per date.
-- Outputs 14 columns (C–P): Head Office + 13 branches.
...
line_amounts AS (
    ...
    FROM {{ ref('stg__map_total_expense') }}
    {% if var("target_date") != "" %}
