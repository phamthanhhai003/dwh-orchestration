{{config(
    ...
    SELECT
        DISTINCT CAST(
            DATE_TRUNC('month', CAST(load_date AS DATE)) AS DATE
        ) AS month_start
    FROM
        {{ ref('final_assets_report') }}
    {% if var("target_date", "") != "" %}
...
periods AS (
    ...
    FROM
        all_months
    UNION
    ALL -- Nhánh 2: Kỳ tháng trước (PREV)
    SELECT
...
base AS (
    ...
    FROM
        period_order_spine pos
        LEFT JOIN {{ref('final_assets_report')}} f ON pos.order_id = f.order_id
...
FROM
    base b
    LEFT JOIN code_dim d ON b.order_id = d.order_id
